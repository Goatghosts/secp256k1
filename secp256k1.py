...
from utils import get_public_key, decompress_public_key
...
# ==============================================================================
ice.privatekey_to_coinaddress.argtypes = [
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_bool,
    ctypes.c_char_p,
]  # intcoin,012,comp,pvk
...
# ==============================================================================
ice.privatekey_loop_h160.argtypes = [
    ctypes.c_ulonglong,
    ctypes.c_int,
    ctypes.c_bool,
    ctypes.c_char_p,
    ctypes.c_char_p,
]  # num,012,comp,pvk,ret
# ==============================================================================
ice.privatekey_loop_h160_sse.argtypes = [
    ctypes.c_ulonglong,
    ctypes.c_int,
    ctypes.c_bool,
    ctypes.c_char_p,
...
# ==============================================================================
ice.pbkdf2_hmac_sha512_list.argtypes = [
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_ulonglong,
    ctypes.c_int,
    ctypes.c_ulonglong,
]  # ret,words,len,mnem_size,total
# ==============================================================================
...
# ==============================================================================
ice.point_loop_subtraction.argtypes = [
    ctypes.c_ulonglong,
    ctypes.c_char_p,
    ctypes.c_char_p,
    ctypes.c_char_p,
...
# ==============================================================================
ice.point_loop_addition.argtypes = [
    ctypes.c_ulonglong,
    ctypes.c_char_p,
    ctypes.c_char_p,
    ctypes.c_char_p,
...
ice.bloom_check_add.argtypes = [
    ...
    ctypes.c_ubyte,
    ctypes.c_char_p,
]  # buff, len, 0_1, _bits, _hashes, _bf
ice.bloom_check_add.restype = ctypes.c_int
# ==============================================================================
ice.bloom_batch_add.argtypes = [
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_int,
    ctypes.c_int,
    ...
    ctypes.c_ubyte,
    ctypes.c_char_p,
]  # chunk, buff, len, 0_1, _bits, _hashes, _bf
# ==============================================================================
ice.bloom_check_add_mcpu.argtypes = [
    ctypes.c_void_p,
    ctypes.c_ulonglong,
    ctypes.c_char_p,
    ctypes.c_int,
    ...
    ctypes.c_ubyte,
    ctypes.c_char_p,
]  # buff, num_items, found_array, len, 0_1, _bits, _hashes, _bf
# ==============================================================================
...
def custom_scalar_multiplication(pvk_int):
    if pvk_int < 0:
        pvk_int = N + pvk_int
    res = _scalar_multiplication(pvk_int)
    return decompress_public_key(bytes(bytearray(res)))
...
def custom_point_increment(x, y):
    pubkey_bytes = get_public_key(x, y)
    res = _point_increment(pubkey_bytes)
    return decompress_public_key(bytes(bytearray(res)))
...
def bech32_address_decode(addr, coin_type=0):
    ...
    if len(addr) > 50:
        print("[Error] Bech32 p2wsh Not Supported. Result Truncated")
    h160 = (b"\x00") * 20
...
def fl(sstr, length=64):
    ...
    if type(sstr) == int:
        ...
        print("[Error] Input format [Integer] [Hex] [Bytes] allowed only. Detected : ", type(sstr))
    return fixed
...
def custom_point_addition(x1, y1, x2, y2):
    pubkey1_bytes = get_public_key(x1, y1)
    pubkey2_bytes = get_public_key(x2, y2)
    res = _point_addition(pubkey1_bytes, pubkey2_bytes)
    return decompress_public_key(bytes(bytearray(res)))
...
def check_in_bloom(this_line, _bits, _hashes, _bf):
    if type(this_line) != bytes:
        tt = str(this_line).encode("utf-8")
    else:
        tt = this_line
    if ice.bloom_check_add(tt, len(tt), 0, _bits, _hashes, _bf) > 0:
        return True
    else:
        return False
